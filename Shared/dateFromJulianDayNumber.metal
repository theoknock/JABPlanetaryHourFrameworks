...
kernel void dateFromJulianDayNumber(const device float *julianDayValue [[ buffer(0) ]],
...
{
    int JulianDayNumber = (int)floor(julianDayValue[id]);
    ...
    float timeValue = ((julianDayValue[id] - floor(julianDayValue[id])) + 0.5) * 24;
