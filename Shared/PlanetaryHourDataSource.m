...
Planet(^planetForDay)(NSDate * _Nullable) = ^(NSDate * _Nullable date) {
    ...
    //    NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    Planet planet = [[NSCalendar currentCalendar] component:NSCalendarUnitWeekday fromDate:date] - 1;
    ...
    validateLocation = ^(void) {
            ...
            !CLLocationCoordinate2DIsValid(PlanetaryHourDataSource.data.locationManager.location.coordinate))
        {
            ...
            ^void (void(^solarCycleCompletionBlock)(NSDictionary<NSNumber *, NSDate *> *),
                   ...
                   int (^ _Nonnull solarCycleDataProviderDate)(NSDate * _Nonnull),
                   CLLocationCoordinate2D (^ _Nonnull solarCycleDataProviderLocation)(CLLocation * _Nullable),
                   NSDate *(^dateFromJulianDayNumber)(double),
                   NSDictionary<NSNumber *,NSDate *> * _Nonnull (^ _Nonnull solarCycleDataProvider)(int, CLLocationCoordinate2D, NSDate *(^)(double)))
            {
                ...
                solarCycleDates = ^(NSDictionary<NSNumber *, NSDate *> *incomingTwilightDates)
                {
                    ...
                    calculatePlanetaryHourData = ^(NSDictionary<NSNumber *, NSDate *> *solarCycleData)
                    {
                        ...
                        NSTimeInterval startDateInterval  = hourDuration * mod_hour;
                        NSDate *startDate                 = [[NSDate alloc] initWithTimeInterval:startDateInterval sinceDate:sinceDate];
                        NSTimeInterval endDateInterval    = hourDuration * (mod_hour + 1);
                        NSDate *endDate                   = [[NSDate alloc] initWithTimeInterval:endDateInterval sinceDate:sinceDate];
                        ...
                        [data enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL * _Nonnull stop) {
                            switch (idx) {
                                case StartDate:
                                {
                                    [planetaryHourData setObject:startDate forKey:@(StartDate)];
                                    break;
                                }
                                case EndDate:
                                {
                                    [planetaryHourData setObject:endDate forKey:@(EndDate)];
                                    break;
                                }
                                case Symbol:
                                ...
                                case StartCoordinate:
                                    [planetaryHourData setObject:start_coordinate forKey:@(StartCoordinate)];
                                    break;
                        ...
                        dispatch_async(dispatch_get_global_queue(0, DISPATCH_QUEUE_PRIORITY_HIGH), ^{
                            ...
                            if (currentHour != NSNotFound)
                            {
                                ...
                                if (currentIndex != NSNotFound)
                                {
                                    ...
                                    if (planetaryHoursCalculationsCompletionBlock != nil) planetaryHoursCalculationsCompletionBlock(planetaryHoursDataArrays);
                ...
                }; solarCycleDates(solarCycleDataProvider(solarCycleDataProviderDate(date), solarCycleDataProviderLocation(PlanetaryHourDataSource.data.locationManager.location), dateFromJulianDayNumber));
            } (solarCycleCompletionBlock,
               [NSDate date],
               PlanetaryHourDataSource.data.locationManager.location,
...
//    if (planetaryHourDataSourceStartCompletionBlock != nil) planetaryHourDataSourceStartCompletionBlock();
    validateLocation();
    //    dispatch_async(dispatch_get_main_queue(), validateLocation);
